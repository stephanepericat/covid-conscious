<template>
  <div class="tcl-card rounded-lg overflow-hidden shadow-md">
    <div v-if="!hideThumbnail" class="aspect-video">
      <NuxtLink class="no-underline relative" :target="target" :to="url">
        <SanityImage
          v-if="visual"
          :asset-id="visual"
          :alt="title"
        ...
        <NuxtImg v-else src="/tcl-fallback-169.jpg" class="w-full" />
        <Icon
          v-if="isVideo"
    ...
    <div class="p-6">
      <h4 class="leading-snug">
        <NuxtLink class="no-underline" :target="target" :to="url">
          {{ title }}
        </NuxtLink>
      </h4>
      <p v-if="date" class="text-sm uppercase tracking-widest">
        {{
          ...
          -
          {{
            format(new Date(convertTs(end)), LOCALIZED_DATE_FORMAT, {
              locale: getDateLocale(locale),
            })
          }}</span
        >
