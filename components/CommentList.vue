<template>
  <div class="sf-comment-list" :class="{ pending }">
    <ILoader v-if="pending" class="sf-comment-list__loader" />
    <p
      v-else-if="!pending && !totalItems"
      v-text="$t('forum.comments.noComments')"
    />
    <template v-else>
      ...
      <IListGroup size="sm" :border="false">
        ...
        >
          <div class="sf-comment-list__item--contents">
            <p class="sf-comment-list__body" v-text="comment.content" />
          ...
          <em>
            <span
              ><NuxtLink
                :to="localePath(`${rootPath}/user/${comment.author.id}`)"
                >@{{ comment.author.profile.name }}</NuxtLink
              >
              &bullet;
            </span>
            <span>{{
              format(new Date(comment.createdAt), DEFAULT_DATE_FORMAT)
            }}</span>
          </em>
        </IListGroupItem>
      </IListGroup>
...
const showDeleteAction = (userId) =>
  userStore.info?.id && userId === userStore.info.id
