<template>
  <div class="sf-post-editor">
    ...
    <IForm v-else v-model="form" @submit="showConfirmation = true">
      <IFormGroup required>
        ...
        <IInput
          ...
          autofocus
        />
        <IFormError for="title" :visible="['invalid']" />
      </IFormGroup>
      <IFormGroup required>
        ...
        <ClientOnly>
          ...
          />
          <IFormError for="contents" :visible="['invalid']" />
          <template #fallback>
      ...
      <IFormGroup>
        <IButton type="submit" block :disabled="buttonDisabled">
          <Icon
            v-if="isSubmitting"
    ...
    <IModal v-model="showConfirmation" size="lg">
      ...
      <template #footer>
        <div class="sf-post-editor__modal--footer">
          <IButton
            class="sf-post-editor__modal--footer-action"
...
const categories = computed(
  () =>
    topics.value?.map(({ id, name }) => ({
      id,
      label: t(`forum.create.categories.${name}`),
    })) || [],
)
...
const onContentsUpdate = () => {
  ...
  form.value.contents.errors = hasContent.value
    ? []
    : [form.value.contents.validators[0]]
