<template>
  <div class="sf-my-posts-page">
    <h1 class="sf-my-posts-page__title" v-text="$t('forum.posts.title')" />
    <!-- <p>{{  posts.entries }}</p> -->
    <ILoader
      v-if="loading || reloading"
      class="sf-my-posts-page__loader"
      size="sm"
    />
    <p
      v-else-if="!loading && !reloading && !posts.entries?.length"
      v-text="$t('forum.posts.noPosts')"
    />
    <template v-else>
      ...
      <ITable class="sf-my-posts-page__table">
        <thead>
          <tr>
            ...
            <th
              class="sf-my-posts-page__table--flex"
              v-text="$t(`forum.posts.table.title`)"
            />
            <th
              class="sf-my-posts-page__table--fixed"
              v-text="$t(`forum.posts.table.topic`)"
            />
            <th
              class="sf-my-posts-page__table--fixed"
              v-text="$t(`forum.posts.table.published`)"
            />
          </tr>
        </thead>
        <tbody class="sf-my-posts-page__table--body">
          <tr v-for="post in posts.entries" :key="post.id">
            ...
            <td>{{ post.title }}</td>
            <td v-if="post.categories.length > 0">
      ...
      <IModal v-model="showConfirmation">
        ...
        <p
          v-text="
            $t('forum.posts.modal.description', { amount: selection.length })
          "
        />
        ...
        <template #footer>
          <div class="sf-my-posts-page__modal--footer">
            <IButton
              class="sf-my-posts-page__modal--footer-action"
...
const selectedSome = computed(
  () =>
    selection.value.length > 0 && selection.value.length < itemsPerPage.value,
)
const selectedAll = computed(
  () => selection.value.length === itemsPerPage.value,
)
