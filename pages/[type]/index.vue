<template>
  <div class="type-page" :class="{ pending, type }">
    <div class="flex justify-center" v-if="disableRendering">
      <NeedLogin class="my-8 md:my-16" url="/auth/auth0" />
    </div>
    ...
    <template v-else>
      ...
      <div v-if="showInstructions" class="type-page__instructions">
        ...
        <p class="type-page__instructions--message">
      ...
      <template v-else>
        ...
        <PublicationList
          :hide-thumbnail="
            isResource(type) && $appSettings.HIDE_RESOURCES_THUMBNAIL
          "
          :items="visibleItems"
...
const { data, pending } = disableRendering.value
  ...
  : useLazySanityQuery(publicationsByTypeQuery, {
      locale,
...
const filterCountries = computed(() => {
  ...
  const matches = selectedCategory.value
    ? results.value.filter(
        (result) => result.categoryUri === selectedCategory.value,
      )
    : results.value
...
const filterCities = computed(() => {
  ...
  const matches = results.value.filter(
    (result) => result.countryCode === selectedCountry.value,
  )
  ...
  return _.sortBy(
    _.uniqBy(
      contentTypes.map((type) => ({
        label: t(`covidnet.types.${type.toLowerCase()}`),
        id: type,
      })),
      'id',
    ),
...
const matches = computed(() => {
  ...
  if (isCovidnet(type) && selectedContentType.value) {
    items = items.filter(
      (result) => result.contentType === selectedContentType.value,
    )
  }
...
const visibleItems = computed(() =>
  matches.value.slice(startItem.value, endItem.value),
)
